...
import { Objectives, linkedKeyResults } from "../AdminPanel/objectives";
import EditOkr from "../AdminPanel/EditOkr";

const MileStone = ({ milestone, inde, openModal }) => {
  return (
    <div
      className={
        "border py-2 px-2 flex items-center justify-between rounded-md divide-x divide-gray-200 " +
        ((milestone.value / milestone.target_value) * 100 > 75
          ? "bg-green-300"
          : (milestone.value / milestone.target_value) * 100 > 50
          ? "bg-amber-300"
          : (milestone.value / milestone.target_value) * 100 > 20
          ? "bg-orange-300"
          : "bg-red-300")
      }
      key={inde}
      ...
      <div className="flex items-center space-x-2 divide-x divide-gray-200">
        <div className="px-1 py-1">
          <div
            ...
            style={{
              "--value": (milestone.value / milestone.target_value) * 100,
              "--size": "2.5rem",
              "--thickness": "1px",
            }}
          >
            {((milestone.value / milestone.target_value) * 100).toFixed(0)}%
          </div>
        </div>
      ...
      <div className="flex items-center divide-x divide-x-200">
        ...
        <div className="flex flex-col items-center space-y-2 px-1">
          <p className="text-green-700">
            Assigned:
            {moment(milestone.assigned_date).format("MMM Do YY")}
          </p>
          <p className="text-red-700">
            Due:
            {moment(milestone.due_date).format("MMM Do YY")}
          </p>
        </div>
      </div>
    </div>
                    ...
                    <Disclosure.Panel className="px-4 pt-4 pb-2 text-sm text-gray-800 bg-white flex flex-col rounded-md">
                      <div className="flex flex-col space-y-2 w-full pr-40 pl-20">
                        {objective.key_results.map((milestone, inde) =>
                          linkedKeyResults.findIndex(
                            (x) => x.linked_to === milestone._id
                          ) === -1 ? (
                            <MileStone
                              milestone={milestone}
                              inde={index}
                              ...
                              {({ open }) => (
                                <>
                                  <Disclosure.Button className="flex w-full justify-between rounded-lg bg-purple-100 px-4 py-2 text-left text-sm font-medium text-purple-900 hover:bg-purple-200 focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
                                    <div
                                      className={
                                        "border py-2 px-2 flex items-center w-full justify-between rounded-md divide-x divide-gray-200 " +
                                        ((milestone.value /
